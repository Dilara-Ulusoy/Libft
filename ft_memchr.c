...
If c is found, the function returns memory address of index i where c is found.
Unsigned char represents a byte of memory. 
By casting to unsigned char, we can access the memory block byte by byte,
which is necessary for searching for character (c) within the memory block.
...
This allows us to accurately compare each byte of the memory block 
